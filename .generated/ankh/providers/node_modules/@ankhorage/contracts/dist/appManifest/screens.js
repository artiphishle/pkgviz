...
const PERMISSION_NAME_SET = new Set(ANKHORAGE_PERMISSION_NAMES);
const CAPABILITY_NAME_SET = new Set(ANKHORAGE_CAPABILITY_NAMES);
...
export function isThemeRegistry(value) {
    return (isRecord(value) &&
        Object.entries(value).every(([registryKey, theme]) => isThemeConfig(theme) && isRecord(theme) && registryKey === theme.id));
}
/*** Validate screens in the authored screen registry. */
export function isScreenRegistry(value) {
    return (isRecord(value) &&
        Object.entries(value).every(([registryKey, screen]) => isScreenSpec(screen) && isRecord(screen) && registryKey === screen.id));
}
/*** Validate optional light and dark splash-screen configuration. */
export function isSplashScreenSpec(value) {
    return (isSplashScreenModeSpec(value) &&
        isRecord(value) &&
...
function isUiNodeRepeatSpec(value) {
    return (isRecord(value) &&
...
function isRequirementSet(value, allowedNames) {
