...
export function isDataEndpointRegistry(value) {
    return (isRecord(value) &&
        Object.entries(value).every(([endpointId, endpoint]) => isDataEndpointConfig(endpoint) && isRecord(endpoint) && endpoint.id === endpointId));
}
/*** Validate every schema in the data schema registry. */
export function isDataSchemaRegistry(value) {
...
export function isAdapterRef(value) {
    return (isRecord(value) &&
        typeof value.id === 'string' &&
        typeof value.kind === 'string' &&
...
function isDataEndpointConfig(value) {
    return (isRecord(value) &&
        ...
        Object.entries(value.operations).every(([operationId, operation]) => isDataOperationConfig(operation) && isRecord(operation) && operation.id === operationId) &&
...
function isOptionalSchemaCollections(value) {
    return ((value.required === undefined || isStringArray(value.required)) &&
...
function isOptionalSchemaComposition(value) {
...
function isDataSchemaRef(value) {
