...
import { isAppNavigatorManifest, isManifestMetadata, isScreenRegistry, isSplashScreenSpec, isThemeRegistry, } from './screens';
...
function hasRequiredManifestKeys(value) {
    return Object.entries(APP_MANIFEST_KEY_POLICY).every(([key, policy]) => policy === 'optional' || key in value);
}
/*** Validate the canonical application manifest field policy. */
function isActiveThemeMode(value) {
    return value === undefined || value === 'dark' || value === 'light';
}
/*** Validate the canonical application manifest field policy. */
function isRepositoryManifest(value) {
    return (isRecord(value) &&
...
function isPresentation(value) {
    return (isThemeRegistry(value.themes) &&
...
function isOptionalCapabilities(value) {
    return ((value.deploy === undefined || isAppDeployManifest(value.deploy)) &&
...
//# sourceMappingURL=isAppManifest.js.map