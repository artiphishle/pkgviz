...
export type InfraCapability = (typeof INFRA_ADAPTER_CATALOG)[InfraAdapterId]['capabilities'][number];
export type InfraProviderFor<C extends InfraCapability> = {
    [P in InfraAdapterId]: C extends (typeof INFRA_ADAPTER_CATALOG)[P]['capabilities'][number] ? P : never;
}[InfraAdapterId];
...
export interface InfraSecretStoreConfigMap {
    readonly 'supabase-vault': {
        readonly schema?: string;
    };
}
export type InfraDatabaseSpec = {
    [P in InfraProviderFor<'database'>]: {
        readonly provider: P;
    } & InfraDatabaseConfigMap[P];
...
export type InfraAuthSpec = {
    [P in InfraProviderFor<'auth'>]: {
        readonly provider: P;
    } & InfraAuthConfigMap[P];
}[InfraProviderFor<'auth'>];
export type InfraAuthzSpec = {
    [P in InfraProviderFor<'authz'>]: {
        readonly provider: P;
    } & InfraAuthzConfigMap[P];
}[InfraProviderFor<'authz'>];
export type InfraSecretStoreSpec = {
    [P in InfraProviderFor<'secretStore'>]: {
        readonly provider: P;
    } & InfraSecretStoreConfigMap[P];
...
export interface InfraManifest {
    readonly environments: Readonly<Record<'local', InfraEnvironmentSpec> & Partial<Record<Exclude<AppEnvironmentId, 'local'>, InfraEnvironmentSpec>>>;
    readonly apis?: ApiDefinitionRegistry;
    readonly modules: InfraModuleRegistry;
}
//# sourceMappingURL=infraManifest.d.ts.map