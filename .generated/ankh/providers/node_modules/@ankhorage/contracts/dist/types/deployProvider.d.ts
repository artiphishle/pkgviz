...
export declare const DEPLOYMENT_CAPABILITIES: readonly ["provision", "prepare", "build", "publish", "verify"];
export type DeploymentCapability = (typeof DEPLOYMENT_CAPABILITIES)[number];
export declare const DEPLOYMENT_PROVIDER_CAPABILITY_IDS: readonly ["setup", "web-publish", "android-build", "android-publish", "ios-build", "ios-publish", "store-listing", "monetization", "release"];
...
export type DeploymentSecretResolver = (reference: DeploymentCredentialReference) => Promise<DeploymentSecretMaterial | null>;
...
export type DeploymentProvisioningRequirement = DeploymentAutomatedProvisioningRequirement | {
    readonly type: 'authentication';
    readonly action: DeploymentAuthenticationRequiredAction;
} | {
    readonly type: 'manual-action';
    readonly action: DeploymentManualAction;
};
...
export interface WebDeploymentPublisher {
    publishAsync(request: WebDeploymentPublishRequest): Promise<DeploymentProviderResult<WebDeploymentPublication>>;
}
export declare const ANDROID_DEPLOYMENT_TRACKS: readonly ["internal", "alpha", "beta", "production"];
export type AndroidDeploymentTrack = (typeof ANDROID_DEPLOYMENT_TRACKS)[number];
export declare const ANDROID_RELEASE_STATUSES: readonly ["draft", "completed"];
...
export interface AndroidDeploymentBuilder {
    inspectAsync(request: AndroidBuildInspectionRequest): Promise<DeploymentProviderResult<AndroidBuildInspection>>;
...
export interface AndroidDeploymentPublisher {
    inspectAsync(request: AndroidPublishInspectionRequest): Promise<DeploymentProviderResult<AndroidPublishInspection>>;
    publishAsync(request: AndroidPublishRequest): Promise<DeploymentProviderResult<AndroidDeploymentPublication>>;
    verifyAsync(request: AndroidPublishRequest): Promise<DeploymentProviderResult<AndroidPublishInspection>>;
}
...
export interface IosDeploymentPublisher {
    inspectAsync(request: IosPublishInspectionRequest): Promise<DeploymentProviderResult<IosPublishInspection>>;
...
//# sourceMappingURL=deployProvider.d.ts.map