...
function AvatarGroupInner({ themeId: _themeId, mode: _mode, interactionPolicy: _interactionPolicy, testID, items, max = 4, size = 's', shape = 'circle', overflowLabel = defaultOverflowLabel, }) {
    ...
    const renderItem = (item, index) => (<View key={item.id ?? `${index}`} ml={index === 0 ? 0 : -overlap} radius="full" borderWidth={2} borderColor={borderColor}>
    ...
    return (<View align="center" direction="row" testID={testID} wrap="nowrap">
      {visibleItems.map(renderItem)}
      {overflowCount > 0 ? (<View ml={visibleItems.length === 0 ? 0 : -overlap} radius="full" borderWidth={2} borderColor={borderColor}>
