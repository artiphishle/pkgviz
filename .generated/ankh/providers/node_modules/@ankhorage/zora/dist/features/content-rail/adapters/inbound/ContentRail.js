...
function ContentRailCarousel({ themeId: _themeId, mode: _mode, interactionPolicy, children, itemSize = 'responsive', gap = 'm', padding = 'm', peek = 32, showControls = true, bounces, showsScrollIndicator = false, direction = 'auto', motion = 'system', accessibilityLabel = 'Content rail', previousLabel = 'Previous items', nextLabel = 'Next items', onControlPress, onVisibleRangeChange, testID, }) {
    ...
    const emitVisibleRange = React.useCallback((nextOffset) => {
        ...
        if (previous?.firstVisibleIndex === event.firstVisibleIndex &&
    ...
    return (<View gap="s" testID={testID}>
      {showControls && hasOverflow ? (<View direction="row" justify="space-between" wrap="nowrap">
          <Text accessibilityLiveRegion="polite" emphasis="muted" variant="caption">
            {rangeLabel}
          </Text>
          <View direction="row" gap="xs" wrap="nowrap">
            <IconButton disabled={passive || !railState.canGoPrevious} icon={{ name: isRtl ? 'chevron-forward' : 'chevron-back' }} interactionPolicy={interactionPolicy} label={previousLabel} onPress={() => scrollToIndex(railState.anchorIndex - 1, 'previous')} size="l"/>
            <IconButton disabled={passive || !railState.canGoNext} icon={{ name: isRtl ? 'chevron-back' : 'chevron-forward' }} interactionPolicy={interactionPolicy} label={nextLabel} onPress={() => scrollToIndex(railState.anchorIndex + 1, 'next')} size="l"/>
          </View>
        </View>) : null}
      <ScrollView accessibilityLabel={accessibilityLabel} accessibilityRole="list" contentContainerStyle={contentStyle} decelerationRate="fast" disableIntervalMomentum horizontal onContentSizeChange={(width) => {
            if (isRtl)
        ...
        }} onScroll={handleScroll} ref={scrollViewRef} scrollEnabled={!passive} scrollEventThrottle={16} bounces={bounces} showsHorizontalScrollIndicator={showsScrollIndicator} snapToInterval={itemWidth > 0 ? itemWidth + gapValue : undefined}>
        {items.map((item, index) => (<View key={index} style={{ flexShrink: 0, width: itemWidth || undefined }}>
            {item}
          </View>))}
      </ScrollView>
    </View>);
...
//# sourceMappingURL=ContentRail.js.map