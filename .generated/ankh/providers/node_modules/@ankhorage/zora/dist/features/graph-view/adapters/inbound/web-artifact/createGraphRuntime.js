...
export function createGraphRuntime(container, callbacksRef) {
    const cy = cytoscape({
        container,
        elements: [],
        minZoom: 0.05,
...
function createRuntimeState(cy, container, callbacksRef) {
    const fitPaddingRef = { current: 50 };
    const viewport = createGraphController(cy, fitPaddingRef, (options) => requestOptimizedFit(state, options));
    const { controller } = viewport;
    const layoutRunningRef = { current: false };
    const readyRef = { current: false };
    const renderedNodeListeners = new Set();
    const stateBase = {
        fitRequestRef: { current: null },
        optimizedSpacingRef: { current: null },
        callbacksRef,
        controller,
        viewport,
        cy,
        ...
        layoutRunningRef,
        nodeSizes: new Map(),
        labelSizedNodeIds: new Set(),
        loopSizer: createGraphLoopSizer(cy),
        readyRef,
        settledTopologyRef: { current: null },
        relayoutScheduledRef: { current: false },
        renderedNodeListeners,
    };
    const unbindEvents = bindGraphEvents(cy, callbacksRef, controller, () => emitRenderedNodes(stateBase));
 ...
 */
function updateRuntime(state, input) {
    if (state.cy.destroyed())
        return;
    const last = state.latestUpdateRef.current;
    const acceptsSpacing = state.optimizedSpacingRef.current !== null &&
    ...
    state.cy.batch(() => {
        ...
        if (previous?.styleRules !== input.styleRules ||
    ...
    sizeGraphNodesToLabels(state.cy, state.labelSizedNodeIds, input.sizeNodesToLabels === true && !input.richNodeRendering);
    ...
    if (!relayout) {
        if (previous?.zoomMode !== input.zoomMode ||
 ...
 */
function completeCurrentLayout(state, generation) {
    scheduleGraphFrame(() => {
        if (state.cy.destroyed() || generation !== state.generationRef.current)
            return;
        state.loopSizer.update();
        ...
        const shouldFit = shouldFitGraphAfterLayout(state.readyRef.current, topology !== state.settledTopologyRef.current);
        state.viewport.settle(shouldFit);
        state.settledTopologyRef.current = topology;
        const fitRequest = state.fitRequestRef.current;
        state.fitRequestRef.current = null;
        if (fitRequest)
            requestOptimizedFit(state, fitRequest);
        emitRenderedNodes(state);
 ...
 */
function requestOptimizedFit(state, options) {
    if (state.cy.destroyed())
        return;
    if (state.layoutRunningRef.current) {
...
function stopCurrentLayout(state) {
...
function applyGraphStyles(cy, rules, richNodeRendering) {
    const styles = [...(rules ?? [])];
 ...
 */
function setSelectedNodeIds(state, nodeIds) {
    if (state.cy.destroyed() || nodeIds === undefined)
        return;
 ...
 */
function setNodeSize(state, id, size) {
...
function applyKnownNodeSizes(state) {
    for (const [id, size] of state.nodeSizes)
        applyNodeSize(state.cy, id, size);
}
/*** Apply one measured model-space size as a Cytoscape per-node style bypass. */
function applyNodeSize(cy, id, size) {
 ...
 */
function scheduleMeasuredNodeRelayout(state) {
    if (state.relayoutScheduledRef.current)
        return;
    state.relayoutScheduledRef.current = true;
    scheduleGraphFrame(() => {
        state.relayoutScheduledRef.current = false;
        const input = state.latestUpdateRef.current;
        if (input === null || state.cy.destroyed())
            return;
        stopCurrentLayout(state);
        startCurrentLayout(state, input);
    });
...
function subscribeRenderedNodes(state, listener) {
 ...
 */
function emitRenderedNodes(state) {
    if (state.renderedNodeListeners.size === 0)
        return;
    const nodes = readGraphRenderedNodes(state.cy, state.hoveredNodeIds);
    for (const listener of state.renderedNodeListeners)
        listener(nodes);
}
/*** Translate one rich-overlay event back into normal graph selection and callbacks. */
function handleOverlayNodeEvent(state, id, type) {
    if (type === 'pointer-enter')
        state.hoveredNodeIds.add(id);
    if (type === 'pointer-leave')
        state.hoveredNodeIds.delete(id);
    if (type === 'press')
...
//# sourceMappingURL=createGraphRuntime.js.map