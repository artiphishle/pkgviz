...
export async function createWebDesiredStateWithNodeAsync(input) {
    ...
    const previous = await readFile(desiredPath, 'utf8').catch((error) => {
    ...
    if (next !== previous)
        await replaceDesiredStateAsync(desiredPath, next);
    try {
        ...
        if (next !== previous) {
            try {
                if (previous === undefined)
                    await rm(desiredPath);
                else
                    await replaceDesiredStateAsync(desiredPath, previous);
            }
            catch (rollbackError) {
...
async function replaceDesiredStateAsync(path, content) {
...
//# sourceMappingURL=createWebDesiredStateWithNodeAsync.js.map