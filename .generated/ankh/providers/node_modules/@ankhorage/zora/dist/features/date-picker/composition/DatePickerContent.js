...
export function DatePickerContent({ description, interactionPolicy, label, maxDate, minDate, onDismiss, onSelect, testID, value, }) {
    ...
    return (<View gap="m" p="m" testID={testID ? `${testID}-content` : undefined}>
      <PickerHeader description={description} title={label ?? 'Choose date'}/>
      <DatePickerMonthNavigation displayMonth={displayMonth} interactionPolicy={interactionPolicy} maxDate={resolvedMaxDate} minDate={resolvedMinDate} onMonthChange={setDisplayMonth}/>
      <DatePickerWeekdays />
      <DatePickerCalendar interactionPolicy={interactionPolicy} maxDate={resolvedMaxDate} minDate={resolvedMinDate} monthDays={monthDays} onSelect={onSelect} testID={testID} value={value}/>
...
function PickerHeader({ description, title, }) {
    return (<View gap="xxs">
      <Text align="center" variant="label" weight="semiBold">
        {title}
      </Text>
      {description ? (<Text align="center" emphasis="muted" variant="bodySmall">
          {description}
        </Text>) : null}
    </View>);
}
/*** Renders previous and next month controls for the calendar. */
function DatePickerMonthNavigation({ displayMonth, interactionPolicy, maxDate, minDate, onMonthChange, }) {
    ...
    return (<View align="center" direction="row" justify="space-between">
      <Button disabled={!canNavigateToMonth(previousMonth, minDate, maxDate)} interactionPolicy={interactionPolicy} onPress={() => onMonthChange(previousMonth)} size="s" variant="ghost">
        Previous
      </Button>
      <Text align="center" variant="label" weight="semiBold">
        {formatMonthLabel(displayMonth)}
      </Text>
      <Button disabled={!canNavigateToMonth(nextMonth, minDate, maxDate)} interactionPolicy={interactionPolicy} onPress={() => onMonthChange(nextMonth)} size="s" variant="ghost">
        Next
      </Button>
    </View>);
}
...
function DatePickerWeekdays() {
    return (<View direction="row" justify="space-between">
      {WEEKDAY_LABELS.map((weekday) => (<View key={weekday} width={DAY_CELL_WIDTH}>
...
function DatePickerCalendar({ interactionPolicy, maxDate, minDate, monthDays, onSelect, testID, value, }) {
    return (<View direction="row" gap="xs" wrap="wrap">
      {monthDays.map((day, index) => {
            if (!day)
                return <View key={`empty-${index}`} width={DAY_CELL_WIDTH}/>;
            const selected = isSameLocalDay(value, day);
            return (<View key={day.toISOString()} width={DAY_CELL_WIDTH}>
            <Button color={selected ? 'primary' : 'neutral'} disabled={isDateDisabled(day, minDate, maxDate)} interactionPolicy={interactionPolicy} onPress={() => onSelect(day)} size="s" testID={testID ? `${testID}-day-${day.getDate()}` : undefined} variant={selected ? 'solid' : 'ghost'}>
              {day.getDate()}
            </Button>
          </View>);
        })}
...
function canNavigateToMonth(month, minDate, maxDate) {
    const firstDay = new Date(month.getFullYear(), month.getMonth(), 1);
