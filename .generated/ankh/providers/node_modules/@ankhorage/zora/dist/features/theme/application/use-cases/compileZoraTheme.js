 ...
 */
export function compileZoraTheme(themeConfig, options = {}) {
...
function compileMode(themeConfig, mode, targets) {
...
function resolveSelections(generated, mode, targets) {
    const diagnostics = [];
    const selections = targets.map((request) => {
        ...
        const result = swatch
            ...
            : null;
        if (!swatch)
            diagnostics.push(createMissingSwatchDiagnostic(mode, request));
        else if (!result?.selected)
            diagnostics.push(createUnresolvedTargetDiagnostic(mode, request));
        return { request, result };
    });
    return { diagnostics, selections };
}
/*** Describe a requested swatch that the selected harmony does not generate. */
function createMissingSwatchDiagnostic(mode, request) {
    return {
...
function createUnresolvedTargetDiagnostic(mode, request) {
    return {
...
function createRootProvenance(themeConfig) {
    return [
...
function createModeProvenance(mode, targets) {
...
function outputsAgree(left, right) {
    return isDeepEqual(left, right);
}
//# sourceMappingURL=compileZoraTheme.js.map