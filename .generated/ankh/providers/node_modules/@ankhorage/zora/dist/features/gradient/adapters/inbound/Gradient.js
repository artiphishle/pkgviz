...
function GradientInner({ themeId: _themeId, mode: _mode, interactionPolicy: _interactionPolicy, children, colors, locations, start, end, width = '100%', height, minHeight, radius, p, testID, }) {
    const gradientRenderer = useGradientRenderer();
    return (<View height={height} minHeight={minHeight} radius={radius} testID={testID} width={width} style={{ overflow: 'hidden' }}>
      {React.createElement(gradientRenderer, {
            colors,
            end,
            locations,
            start,
