...
function UploaderInner({ themeId: _themeId, mode: _mode, testID, value = null, onChange, onValueChange, onUploadRequest, onRemoveRequest, onValidationError, uploadState = 'idle', uploadProgress, label = 'File', description, helperText, errorText, type = 'file', accept, maxSizeBytes, required, disabled = false, readOnly = false, validatePicked, onUpload, onRemove, aspectRatio = 1, interactionPolicy, }) {
    ...
    return (<>
      <Field description={description} disabled={disabled} errorText={effectiveError} helperText={helperText} interactionPolicy={interactionPolicy} invalid={Boolean(effectiveError)} label={label} readOnly={readOnly} required={required} testID={testID}>
        <View gap="m">
          <UploadPresentation asset={value} aspectRatio={aspectRatio} type={type}/>

          {resolvedAccept || maxSizeBytes ? (<View gap="xs">
              {resolvedAccept ? (<Text emphasis="muted" variant="caption">
                  Accepted: {resolvedAccept}
                </Text>) : null}
              {maxSizeBytes ? (<Text emphasis="muted" variant="caption">
                  Max size: {formatBytes(maxSizeBytes)}
                </Text>) : null}
            </View>) : null}

          {uploading ? (<View gap="xs">
              <Text emphasis="muted" variant="caption">
                Uploading…
              </Text>
              {uploadProgress !== undefined || progress !== null ? (<Progress max={1} value={clampProgress(uploadProgress ?? progress) ?? 0}/>) : null}
            </View>) : null}

          <View direction={{ base: 'column', md: 'row' }} gap="s">
            <Button disabled={actionsDisabled || uploading || removing} interactionPolicy={interactionPolicy} onPress={() => {
            void handlePick();
        }}>
              {value ? 'Replace file' : 'Select file'}
            </Button>

            {value ? (<Button color="danger" disabled={actionsDisabled || uploading || removing} interactionPolicy={interactionPolicy} loading={removing} variant="outline" onPress={() => {
                void handleRemove();
            }}>
                Remove
              </Button>) : null}

            {canPreviewImage ? (<Button color="neutral" interactionPolicy={interactionPolicy} variant="soft" onPress={() => setPreviewOpen(true)}>
                Preview
              </Button>) : null}
          </View>
        </View>
      ...
      {canPreviewImage ? (<Dialog closeOnBackdrop interactionPolicy={interactionPolicy} onDismiss={() => setPreviewOpen(false)} title="Image preview" visible={previewOpen}>
          <View gap="m">
            <Image aspectRatio={aspectRatio} fit="contain" source={value}/>
            <View direction="row" justify="flex-end">
              <Button color="neutral" interactionPolicy={interactionPolicy} variant="soft" onPress={() => setPreviewOpen(false)}>
                Close
              </Button>
            </View>
          </View>
...
function UploadPresentation({ asset, aspectRatio, type, }) {
    if (type === 'image') {
        return <Image aspectRatio={aspectRatio} source={asset}/>;
    }
    return (<View borderWidth={1} p="m" radius="m">
      <View direction="row" gap="s">
        ...
        <View gap="xs">
          ...
          {asset?.contentType ? (<Text emphasis="muted" variant="caption">
              {asset.contentType}
            </Text>) : null}
        </View>
      </View>
