...
export function createNodeWebComponentArtifactFileSystem(platform = process.platform) {
    return {
        joinPath(...parts) {
            return join(...parts);
        },
        ...
        async commitStageDirectoryAsync(stageDirectory, outputDirectory) {
            const parent = dirname(outputDirectory);
            const previous = await readlink(outputDirectory).catch((error) => {
                if (isMissingFile(error))
                    return undefined;
                throw error;
            });
            const previousDirectory = previous === undefined ? undefined : await realpath(outputDirectory);
            ...
            try {
                await symlink(platform === 'win32' ? stageDirectory : basename(stageDirectory), temporaryLink, platform === 'win32' ? 'junction' : 'dir');
...
async function replaceWindowsJunctionAsync(temporaryLink, outputDirectory) {
...
function isGeneratedDirectoryName(value) {
...
//# sourceMappingURL=createNodeWebComponentArtifactFileSystem.js.map