...
function SelectInner({ themeId: _themeId, mode: _mode, ...props }) {
    const [open, setOpen] = React.useState(false);
    const { select, selectedOption, value } = useSelectController(props);
    const { interactionPolicy, options, testID } = props;
    const selectAndClose = React.useCallback((nextValue) => {
        select(nextValue);
        setOpen(false);
    }, [select]);
    return (<SelectField props={props}>
      <Popover anchor={({ toggle }) => (<SelectTrigger displayLabel={selectedOption?.label} onPress={toggle} props={props}/>)} interactionPolicy={props.interactionPolicy} onOpenChange={setOpen} open={open} placement="bottom-start" testID={testID ? `${testID}-popover` : undefined}>
        <Surface variant="raised">
          <View gap="xs" p="xs">
            {options.map((option) => (<SelectOptionRow interactionPolicy={interactionPolicy} key={option.value} onSelect={selectAndClose} option={option} selected={value === option.value} testID={testID}/>))}
          </View>
        </Surface>
      </Popover>
    </SelectField>);
