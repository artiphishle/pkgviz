...
function ScreenInner({ themeId: _themeId, mode: _mode, interactionPolicy: _interactionPolicy, children, footer, scroll = true, width = 'default', testID, }) {
    const content = (<View alignSelf="center" gap="l" maxWidth={resolvePageMaxWidth(width)} px={{ base: 16, md: 24, lg: 32 }} py="xl" testID={testID} width="100%">
      {children}
      {footer}
    </View>);
    if (!scroll) {
        return (<View bg="background" flex={1} minHeight={0} minWidth={0}>
        {content}
      </View>);
    }
    return (<ScrollView bg="background" flex={1} minHeight={0} minWidth={0}>
      {content}
    </ScrollView>);
}
