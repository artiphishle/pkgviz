...
function renderUnreadCount(unreadCount) {
    ...
    return (<Badge size="s" color="primary">
      {unreadCount}
    </Badge>);
}
function ChatListItemInner({ themeId: _themeId, mode: _mode, interactionPolicy, testID, title, preview, meta, timestamp, avatar, leading, trailing, unread = false, unreadCount, selected = false, disabled = false, compact = false, accessibilityLabel, onPress, }) {
    ...
    const content = ({ pressed, hovered }) => {
        ...
        return (<View bg={styles.bg} borderColor={styles.borderColor} borderWidth={styles.borderWidth} px={padding.px} py={padding.py} radius="m" style={{ opacity: styles.opacity }}>
        <View direction="row" align="center" gap="m" wrap="nowrap">
          {leading ?? (<Avatar initials={avatar?.initials} label={avatar?.label ?? avatarName} name={avatarName} shape={avatar?.shape} size={avatar?.size ?? (compact ? 's' : 'm')} source={avatar?.source} color={avatar?.color}/>)}

          <View flex={1}>
            <View gap="xxs">
              <View direction="row" align="center" gap="s" justify="space-between" wrap="nowrap">
                <View flex={1}>
                  <Text numberOfLines={1} emphasis={disabled ? 'muted' : 'default'} variant="bodySmall" weight={unread || selected ? 'semiBold' : 'medium'}>
                    {title}
                  </Text>
                </View>
                {hasTimestamp ? (<Text numberOfLines={1} color={unread ? 'primary' : undefined} emphasis={unread ? 'default' : 'subtle'} variant="caption" weight={unread ? 'semiBold' : 'regular'}>
                    {timestamp}
                  </Text>) : null}
              </View>

              {hasSecondaryRow ? (<View direction="row" align="center" gap="s" justify="space-between" wrap="nowrap">
                  <View flex={1}>
                    <View gap="xxs">
                      {hasPreview ? (<Text numberOfLines={1} emphasis={unread ? 'default' : 'muted'} variant="bodySmall" weight={unread ? 'medium' : 'regular'}>
                          {preview}
                        </Text>) : null}
                      {hasMeta ? (<Text numberOfLines={1} emphasis="subtle" variant="caption">
                          {meta}
                        </Text>) : null}
                    </View>
                  </View>

                  {hasUnreadCount || hasTrailing ? (<View direction="row" align="center" gap="s" wrap="nowrap">
