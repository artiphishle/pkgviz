...
export async function create(request) {
    try {
        ...
        request.context.writeStdout(`Materialized ZORA ${component} with ${result.components.length} web component(s) at ${result.outputDirectory}\n`);
        return { exitCode: 0 };
    }
    catch (error) {
        request.context.writeStderr(`ZORA create failed: ${error instanceof Error ? error.message : String(error)}\n`);
        return { exitCode: 1 };
    }
}
