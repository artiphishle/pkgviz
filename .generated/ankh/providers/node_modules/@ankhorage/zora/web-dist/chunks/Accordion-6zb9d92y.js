'use client';
// node_modules/@ankhorage/surface/src/internal/focus/useFocusManager.ts
...
function useFocusManager() {
  ...
  const capturePreviousFocus = React.useCallback(() => {
    ...
    previousFocusRef.current = activeElement && typeof activeElement === "object" && "focus" in activeElement ? activeElement : null;
  }, []);
  ...
  const cycleFocus = React.useCallback((container, reverse = false) => {
    ...
    const nextIndex = reverse ? (currentIndex - 1 + focusableElements.length) % focusableElements.length : (currentIndex + 1) % focusableElements.length;
