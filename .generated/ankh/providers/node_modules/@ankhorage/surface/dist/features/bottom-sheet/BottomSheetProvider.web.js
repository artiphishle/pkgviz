...
export function BottomSheetProvider({ children }) {
    ...
    const controller = React.useMemo(() => ({ dismiss, present }), [dismiss, present]);
    React.useEffect(() => {
        if (activeRequest === null)
            return;
        activeRequest.onIndexChange?.(resolveInitialIndex(activeRequest));
    }, [activeRequest]);
    return (<BottomSheetContext value={controller}>
      {children}
...
function BottomSheetWebHost({ activeRequest, dismiss }) {
    if (activeRequest === null)
        return null;
    return (<Modal animationType="slide" onRequestClose={dismiss} transparent visible>
      <View style={styles.overlay}>
        <Pressable accessibilityLabel="Close bottom sheet" disabled={activeRequest.dismissOnBackdropPress === false} onPress={activeRequest.dismissOnBackdropPress === false ? undefined : dismiss} style={styles.backdrop}/>
        <Surface radius="l" style={[styles.sheet, resolveSheetSize(activeRequest)]} variant="raised">
...
function resolveSheetSize(activeRequest) {
...
function resolveInitialIndex(activeRequest) {
...
function renderBottomSheetContent(activeRequest) {
    return activeRequest.contentMode === 'direct' ? (activeRequest.content) : (<View>{activeRequest.content}</View>);
}
...
//# sourceMappingURL=BottomSheetProvider.web.js.map