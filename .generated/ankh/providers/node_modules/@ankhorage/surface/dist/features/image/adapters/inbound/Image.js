...
export function Image({ source, fallbackSource, alt, accessibilityLabel, width, height, aspectRatio, fit, resizeMode, radius, style, testID, onError, }) {
    ...
    if (!resolvedSource)
        return null;
    return (<ReactNativeImage accessibilityLabel={resolveAccessibilityLabel(alt, accessibilityLabel)} accessibilityRole="image" alt={resolveAlt(alt, accessibilityLabel)} onError={(event) => {
            onError?.(event);
            if (!showFallback && normalizedFallback)
                setFailedPrimaryKey(primaryKey);
        }} resizeMode={resolveResizeMode(fit, resizeMode)} source={resolvedSource} style={[resolveImageStyle(theme, { width, height, aspectRatio, radius }), style]} testID={testID}/>);
}
