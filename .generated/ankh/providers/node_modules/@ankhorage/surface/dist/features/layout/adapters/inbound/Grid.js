...
export function Grid({ children, cols, gap = 0, rowGap, colGap, minItemWidth, ...props }) {
    ...
    return (<View {...props} style={[resolveGridStyle(rowSpacing, colSpacing), props.style]}>
      {React.Children.toArray(children).map((node, index) => (<ReactNativeView key={React.isValidElement(node) && node.key !== null ? String(node.key) : String(index)} style={resolveGridItemStyle(activeMinItemWidth, basisPercent, rowSpacing, colSpacing)}>
          {node}
        </ReactNativeView>))}
    </View>);
}
...
function resolveGridItemStyle(minItemWidth, basisPercent, rowSpacing, colSpacing) {
    const sizing = minItemWidth === undefined
